@if(count($student_name) > 0)
    <h6>Payment History for {{$student_name[0]->student_name}}</h6>

    <?php
    $last_data = $student_name->first();
    ...
    @if(!empty($second_month))
        @if(date('Y-m',strtotime($second_month)) == date('Y-m'))
            ...
            <p style="font-weight: bolder;text-align: center">Total Dues 700/-</p>
            @else
            <?php
            ...
            <p style="font-weight: bolder;text-align: center">Total Dues {{$diff*700}}/-</p>
            @endif
        @else
...
@else
    {{'No payment history found of this student.'}}
@endif